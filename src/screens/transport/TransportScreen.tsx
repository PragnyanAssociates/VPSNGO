// 📂 File: src/screens/transport/TransportScreen.tsx (MODIFIED & CORRECTED)
...
const RouteFormModal = ({ routeToEdit, onClose, onSave }) => {
        ...
        const loadData = async () => {
            if (isEditMode && routeToEdit) {
                try {
                    ...
                    const res = await apiClient.get(`/transport/routes/${routeToEdit.route_id}`);
                    const data = res.data;
                    ...
                    setStops(data.stops.length > 0 ? data.stops.map(s => ({ point: s.point })) : [{ point: '' }, { point: '' }]);
                ...
                } catch (e: any) { Alert.alert("Error", e.response?.data?.message || e.message); onClose(); }
    ...
    const handleStopChange = (text, index) => { const newStops = [...stops]; newStops[index].point = text; setStops(newStops); };
    ...
    const handleFormSubmit = async () => {
        ...
        try {
            ...
            onSave();
            onClose();
        } catch (error: any) { Alert.alert("Error", error.response?.data?.message || "Failed to save route."); } finally { setIsSaving(false); }
    };
...
const TransportListScreen = ({ navigation }) => {
    ...
    const fetchRoutes = useCallback(async () => { setIsLoading(true); try { const response = await apiClient.get('/transport/routes'); setRoutes(response.data); } catch (error: any) { Alert.alert("Error", error.response?.data?.message || "Could not fetch routes."); } finally { setIsLoading(false); } }, []);
...
const RouteDetailScreen = ({ route }) => {
    ...
    useEffect(() => {
        const fetchAndTrack = async (isInitialLoad = false) => { 
            try { 
                ...
                } else { setRouteData(prevData => ({ ...prevData, ...data })); }
                ...
                if (!isNaN(newLat) && !isNaN(newLng)) {
                    ...
                    if (prevCoordsRef.current) {
                        ...
                        if (prevLat !== newLat || prevLng !== newLng) {
                           ...
                           liveMarkerRef.current?.animateMarkerToCoordinate(newCoordinate, 2000);
                        }
                    }
                ...
                if (isInitialLoad) Alert.alert("Error", error.response?.data?.message || error.message);
                else Toast.show('Could not update live location.', Toast.SHORT);
            } finally { if (isInitialLoad) setIsLoading(false); } 
        };
        fetchAndTrack(true);
        const interval = setInterval(() => fetchAndTrack(false), 5000);
        return () => clearInterval(interval);
    }, [routeId]);

    useEffect(() => {
        if (mapRef.current && routeCoordinates.length > 1) {
            setTimeout(() => { mapRef.current.fitToCoordinates(routeCoordinates, { edgePadding: { top: 50, right: 50, bottom: 50, left: 50 }, animated: true }); }, 500); 
        }
    ...
    const MapDisplay = (
        <MapView ref={mapRef} style={StyleSheet.absoluteFillObject} provider={null} mapType="none" initialRegion={routeCoordinates.length > 0 ? { latitude: routeCoordinates[0].latitude, longitude: routeCoordinates[0].longitude, latitudeDelta: 0.0922, longitudeDelta: 0.0421 } : undefined}>
            ...
            {routeData.stops && routeData.stops.map(stop => {
                const stopLat = parseFloat(stop.stop_lat); const stopLng = parseFloat(stop.stop_lng);
                if (!isNaN(stopLat) && !isNaN(stopLng)) { return (<Marker key={`stop-${stop.sno}`} coordinate={{ latitude: stopLat, longitude: stopLng }}><View style={styles.stopMarker}><Text style={styles.stopMarkerText}>{stop.sno}</Text></View></Marker>); }
                return null;
            })}
            {(() => {
                const currentLat = parseFloat(routeData.current_lat); const currentLng = parseFloat(routeData.current_lng);
                if (!isNaN(currentLat) && !isNaN(currentLng)) { return ( <Marker ref={liveMarkerRef} anchor={{ x: 0.5, y: 0.5 }} coordinate={{ latitude: currentLat, longitude: currentLng }} rotation={busHeading} flat={true}><View style={styles.liveMarker}><Icon name="location-arrow" size={18} color="#fff" style={styles.liveMarkerArrow} /></View></Marker> ); }
                return null;
            })()}
        </MapView>
    );
    ...
    return (
        <SafeAreaView style={styles.detailContainerSafe}>
            ...
            {isMapFullScreen && ( <View style={styles.fullScreenMapContainer}>{MapDisplay}<TouchableOpacity style={styles.closeMapButton} onPress={() => setIsMapFullScreen(false)}><Icon name="times" size={24} color="#333" /></TouchableOpacity></View> )}
        </SafeAreaView>
    );
};
...
const AdminTransportPanel = ({ navigation }) => { 
    ...
    const isFocused = useIsFocused();
    const fetchAdminRoutes = useCallback(async () => { setIsLoading(true); try { const response = await apiClient.get('/transport/routes'); setRoutes(response.data); } catch (error: any) { Alert.alert("Error", error.response?.data?.message || "Could not fetch routes."); } finally { setIsLoading(false); } }, []);
    ...
    const handleDelete = async (routeId) => { Alert.alert("Confirm Deletion", "Are you sure you want to delete this route?", [{ text: "Cancel", style: "cancel" }, { text: "Delete", style: "destructive", onPress: async () => { try { await apiClient.delete(`/transport/routes/${routeId}`); Alert.alert("Success", "Route deleted."); fetchAdminRoutes(); } catch (error: any) { Alert.alert("Error", error.response?.data?.message || "Failed to delete route."); } } }]); };
    return (
        <SafeAreaView style={styles.listContainerSafe}>
...
const StudentTransportNavigator = () => ( <Stack.Navigator screenOptions={{ headerStyle: { backgroundColor: '#2c3e50' }, headerTintColor: '#fff', headerTitleStyle: { fontWeight: 'bold' } }}><Stack.Screen name="TransportRoutesList" component={TransportListScreen} options={{ title: 'School Transport' }} /><Stack.Screen name="RouteDetails" component={RouteDetailScreen} options={({ route }) => ({ title: route.params?.routeName || 'Route Details' })} /></Stack.Navigator> );
const AdminNavigator = () => ( <Stack.Navigator screenOptions={{ headerStyle: { backgroundColor: '#2c3e50' }, headerTintColor: '#fff', headerTitleStyle: { fontWeight: 'bold' } }}><Stack.Screen name="AdminTransportPanel" component={AdminTransportPanel} options={{ title: 'Manage Routes' }} /><Stack.Screen name="RouteDetails" component={RouteDetailScreen} options={({ route }) => ({ title: route.params?.routeName || 'Route Details' })} /></Stack.Navigator> );
