...
const TransportScreen = () => {
    ...
    if (user.role === 'admin') return <AdminTransportPanel />;
    return <View style={styles.centered}><Text>Transport module not available for your role.</Text></View>;
};

// --- STUDENT/TEACHER VIEW COMPONENTS ---
const TransportStack = createStackNavigator();
const makeCall = (phoneNumber) => { if (phoneNumber) Linking.openURL(`tel:${phoneNumber}`); };
...
const RouteDetailScreen = ({ route }) => {
    ...
    useEffect(() => { const fetchAndTrack = async () => { try { const response = await fetch(`${API_BASE_URL}/api/transport/routes/${routeId}`); if (!response.ok) throw new Error("Could not fetch route details."); const data = await response.json(); setRouteData(data); } catch (error) { if (isLoading) Alert.alert("Error", error.message); } finally { if (isLoading) setIsLoading(false); } }; fetchAndTrack(); const interval = setInterval(fetchAndTrack, 15000); return () => clearInterval(interval); }, [routeId, isLoading]);
    const decodedPolyline = useMemo(() => { if (routeData?.route_path_polyline) { return decode(routeData.route_path_polyline).map(p => ({ latitude: p[0], longitude: p[1] })); } return []; }, [routeData?.route_path_polyline]);
    const initialMapRegion = useMemo(() => { if (decodedPolyline.length > 0) { let minLat = decodedPolyline[0].latitude, maxLat = decodedPolyline[0].latitude, minLng = decodedPolyline[0].longitude, maxLng = decodedPolyline[0].longitude; for (const point of decodedPolyline) { minLat = Math.min(minLat, point.latitude); maxLat = Math.max(maxLat, point.latitude); minLng = Math.min(minLng, point.longitude); maxLng = Math.max(maxLng, point.longitude); } const midLat = (minLat + maxLat) / 2; const midLng = (minLng + maxLng) / 2; const deltaLat = (maxLat - minLat) * 1.5; const deltaLng = (maxLng - minLng) * 1.5; return { latitude: midLat, longitude: midLng, latitudeDelta: deltaLat, longitudeDelta: deltaLng }; } return null; }, [decodedPolyline]);
    useEffect(() => { if (mapRef.current && routeData?.current_lat && routeData?.current_lng) { mapRef.current.animateToRegion({ latitude: routeData.current_lat, longitude: routeData.current_lng, latitudeDelta: 0.02, longitudeDelta: 0.02 }, 1000); } }, [routeData?.current_lat, routeData?.current_lng]);
    if (isLoading) return <View style={styles.centered}><ActivityIndicator size="large" color="#3498db" /></View>;
    ...
    return (
        <SafeAreaView style={styles.detailContainerSafe}>
            <ScrollView>
                ...
                {initialMapRegion ? ( <TouchableOpacity activeOpacity={0.9} onPress={() => setIsMapFullScreen(true)} style={styles.mapContainer}> <Text style={styles.mapTitle}>Live Bus Tracking</Text> <MapView ref={mapRef} provider={PROVIDER_GOOGLE} style={styles.map} initialRegion={initialMapRegion} scrollEnabled={false} zoomEnabled={false} pitchEnabled={false} rotateEnabled={false}> <Polyline coordinates={decodedPolyline} strokeColor="#e74c3c" strokeWidth={5} /> {routeData.current_lat && <Marker coordinate={{ latitude: routeData.current_lat, longitude: routeData.current_lng }}><View style={styles.busMarker}><Icon name="bus" size={20} color="#fff" /></View></Marker>} </MapView> <View style={styles.mapOverlay}><Text style={styles.mapOverlayText}>Tap to track live</Text></View> </TouchableOpacity> ) : ( <View style={styles.mapPlaceholder}> <Text>Route map is unavailable.</Text> <Text style={{fontSize: 12, color: '#7f8c8d', textAlign: 'center', marginTop: 5}}>This can happen if billing is not enabled on the Google Cloud project.</Text> </View> )}
            ...
            {isMapFullScreen && initialMapRegion && ( <Modal visible={isMapFullScreen} animationType="slide" onRequestClose={() => setIsMapFullScreen(false)}> <SafeAreaView style={{flex: 1}}> <TouchableOpacity style={styles.closeMapButton} onPress={() => setIsMapFullScreen(false)}><Icon name="times" size={24} color="#333" /></TouchableOpacity> <MapView style={StyleSheet.absoluteFill} initialRegion={initialMapRegion} showsUserLocation> <Polyline coordinates={decodedPolyline} strokeColor="#e74c3c" strokeWidth={5} /> {routeData.current_lat && <Marker coordinate={{ latitude: routeData.current_lat, longitude: routeData.current_lng }}><View style={styles.busMarker}><Icon name="bus" size={22} color="#fff" /></View></Marker>} </MapView> </SafeAreaView> </Modal> )}
        </SafeAreaView>
    );
};

// ✅ CORRECTED StudentTransportNavigator (No stray semicolon)
const StudentTransportNavigator = () => (
    ...
    >
        <TransportStack.Screen 
            ...
            options={{ title: 'School Transport' }} 
        />
        <TransportStack.Screen 
            name="RouteDetails" 
            component={RouteDetailScreen} 
            options={({ route }) => ({ title: route.params?.routeName || 'Route Details' })} 
        />
    </TransportStack.Navigator>
)
...
const AdminTransportPanel = () => {
    ...
    return (
        <SafeAreaView style={styles.listContainerSafe}>
            <Text style={styles.adminHeader}>Manage Transport Routes</Text>
            <FlatList data={routes} keyExtractor={item => item.route_id.toString()} renderItem={({ item }) => ( <View style={styles.adminCard}> <Text style={styles.adminCardTitle}>{item.route_name}</Text> <View style={styles.adminCardActions}> <TouchableOpacity onPress={() => handleOpenModal(item)}><Icon name="edit" size={20} color="#3498db" /></TouchableOpacity> <TouchableOpacity onPress={() => handleDelete(item.route_id)}><Icon name="trash" size={20} color="#e74c3c" /></TouchableOpacity> </View> </View> )} ListFooterComponent={ <TouchableOpacity style={styles.adminAddButton} onPress={() => handleOpenModal(null)}><Text style={styles.adminAddButtonText}>+ Add New Route</Text></TouchableOpacity> } onRefresh={fetchAdminRoutes} refreshing={isLoading} ListEmptyComponent={!isLoading && <Text style={styles.emptyText}>No routes created yet.</Text>} />
...
const RouteFormModal = ({ routeToEdit, onClose, onSave }) => {
    const { user } = useAuth();
    const GOOGLE_PLACES_API_KEY = "AIzaSyByYtzA7J3SvVUSuO6uzG7IUOpCSoqK_iM";
    ...
    useEffect(() => { const loadData = async () => { if (isEditMode && routeToEdit) { try { const res = await fetch(`${API_BASE_URL}/api/transport/routes/${routeToEdit.route_id}`); if (!res.ok) throw new Error("Could not load route details for editing."); const data = await res.json(); setRouteName(data.route_name || ''); setDriverName(data.driver_name || ''); setDriverPhone(data.driver_phone || ''); setConductorName(data.conductor_name || ''); setConductorPhone(data.conductor_phone || ''); setStops(data.stops.length > 0 ? data.stops.map(s => ({ point: s.point })) : [{ point: '' }, { point: '' }]); const allCountries = Country.getAllCountries(); const foundCountry = allCountries.find(c => c.name === data.country); if (foundCountry) { setCountry(foundCountry.isoCode); const allStates = State.getStatesOfCountry(foundCountry.isoCode); const foundState = allStates.find(s => s.name === data.state); if (foundState) { setState(foundState.isoCode); setCity(data.city); } } } catch (e) { Alert.alert("Error", e.message); onClose(); } finally { setIsLoading(false); } } else { setRouteName(''); setDriverName(''); setDriverPhone(''); setConductorName(''); setConductorPhone(''); setCountry(null); setState(null); setCity(null); setStops([{ point: '' }, { point: '' }]); } }; loadData(); }, [isEditMode, routeToEdit]);
    ...
    const handleStopChange = (data, details, index) => { const newStops = [...stops]; newStops[index].point = details.formatted_address; setStops(newStops); };
    const addStop = () => setStops([...stops, { point: '' }]);
    const removeStop = (index) => { if (stops.length > 2) setStops(stops.filter((_, i) => i !== index)); };
    const handleFormSubmit = async () => { const countryName = Country.getCountryByCode(country)?.name; const stateName = State.getStateByCodeAndCountry(state, country)?.name; if (!routeName || !driverName || !countryName || !stateName || !city || stops.length < 2 || stops.some(s => !s.point.trim())) { return Alert.alert("Validation Error", "Please fill all required fields: Route Name, Driver Name, Country, State, City, and at least two valid boarding points."); } setIsSaving(true); const payload = { route_name: routeName, driver_name: driverName, driver_phone: driverPhone, conductor_name: conductorName, conductor_phone: conductorPhone, stops, city: city, state: stateName, country: countryName, created_by: user.id }; const url = isEditMode ? `${API_BASE_URL}/api/transport/routes/${routeToEdit.route_id}` : `${API_BASE_URL}/api/transport/routes`; const method = isEditMode ? 'PUT' : 'POST'; try { const response = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) }); const resJson = await response.json(); if (!response.ok) throw new Error(resJson.message || "Failed to save route."); Alert.alert("Success", `Route ${isEditMode ? 'updated' : 'created'}!`); onSave(); onClose(); } catch (error) { Alert.alert("Error", error.message); } finally { setIsSaving(false); } };
    return (
                    ...
                    {isLoading ? <ActivityIndicator size="large" color="#3498db" /> : (
                        <>
                            ...
                            {stops.map((stop, index) => (
                                <View key={index} style={[styles.stopInputContainer, { zIndex: stops.length - index }]}>
                                    <GooglePlacesAutocomplete
                                        placeholder={stop.point || `Stop ${index + 1}`}
                                        onPress={(data, details = null) => { handleStopChange(data, details, index); }}
                                        query={{ key: GOOGLE_PLACES_API_KEY, language: 'en', components: country ? `country:${country}` : undefined }}
                                        fetchDetails={true}
                                        predefinedPlaces={[]}
                                        styles={{ container: { flex: 1 }, textInput: styles.input, listView: { backgroundColor: 'white', borderWidth: 1, borderColor: '#ddd' } }}
                                        textInputProps={{ defaultValue: stop.point, editable: !!country }}
                                    />
...
const styles = StyleSheet.create({
    ...
    mapContainer: { marginHorizontal: 15, marginTop: 15, borderRadius: 12, overflow: 'hidden', elevation: 5, backgroundColor: '#ddd' },
    ...
    mapTitle: { fontSize: 18, fontWeight: 'bold', padding: 15, textAlign: 'center', color: '#34495e', backgroundColor: '#ecf0f1' },
    ...
    busMarker: { backgroundColor: '#3498db', padding: 8, borderRadius: 20, elevation: 5, shadowColor: '#000', shadowOpacity: 0.3, shadowRadius: 3, borderWidth: 2, borderColor: '#fff' },
    ...
    dropdownContainer: { marginBottom: 15, zIndex: 1 },
