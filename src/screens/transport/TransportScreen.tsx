...
const RouteDetailScreen = ({ route }) => {
    ...
    useEffect(() => {
        const fetchAndTrack = async () => { 
            try { 
                ...
                // ✅ **DECODE THE POLYLINE RECEIVED FROM THE BACKEND**
    ...
    // This effect now ONLY triggers when routeCoordinates has data.
    useEffect(() => {
        if (mapRef.current && routeCoordinates.length > 0) {
            // Give the map tiles a moment to load before fitting the route
            setTimeout(() => {
    ...
    }, [routeCoordinates]); // Depends only on the coordinates
    ...
    const MapContent = () => (
        <MapView
            ref={mapRef}
            style={styles.map}
            // ✅ CRITICAL FIX: provider={null} on Android forces the non-Google renderer
        ...
        >
            <UrlTile 
                ...
                maximumZ={19}
                zIndex={-1} // Draw tiles behind other elements
            />
