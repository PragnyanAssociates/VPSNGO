...
import {
  ...
  SafeAreaView, Animated, Easing, Image, FlatList
} from 'react-native';
...
// For full voice support install react-native-audio-recorder-player and implement accordingly
...
const ChatAIScreen = () => {
  ...
  // Send "image" message (save only locally, for full support add upload endpoint)
  const handleImagePick = async () => {
    ...
    launchImageLibrary({ mediaType: 'photo' }, async (response) => {
      ...
      // You can send to backend POST /api/chat/message with type: 'image' and uri/content here
  ...
  // (Optional) handle voice message: integrate with a third-party library and backend image/audio upload as needed.

  // Render custom date header
    ...
    >
      <SafeAreaView style={styles.container}>
        <ImageBackground source={{ uri: BACKGROUND_IMAGE_URI }} style={styles.background}>
          ...
          {/* WhatsApp-style toolbar: attach, image, emoji, mic, send */}
          <View style={styles.inputToolbar}>
            <TouchableOpacity/* For general attachments, implement as needed */>
              <Icon name="attach" size={26} color="#A0A0A0" style={styles.toolbarIcon} />
            </TouchableOpacity>
            {/* Payment (₹) icon UI only */}
            <TouchableOpacity>
              <Icon name="cash-outline" size={26} color="#A0A0A0" style={styles.toolbarIcon} />
            </TouchableOpacity>
            ...
            {/* Voice Icon, UI only (hook up real recording to handleVoiceSend) */}
            <TouchableOpacity>
              <Icon name="mic-outline" size={26} color="#00A884" style={styles.toolbarIcon} />
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.sendButtonContainer}
              onPress={() => handleSendPress(inputText)}
              disabled={inputText.trim() === ''}
              activeOpacity={inputText.trim() === '' ? 1 : 0.75}
            >
              <LinearGradient
          ...
          {showEmojiPicker && (
            <View style={{ position: 'absolute', bottom: 70, left: 0, right: 0, height: 260 }}>
              <EmojiSelector
...
const styles = StyleSheet.create({
  ...
  loadingText: {
    color: '#00A884', fontSize: 17, marginTop: 16, letterSpacing: 0.5, fontWeight: '600',
  },
  header: {
    flexDirection: 'row', alignItems: 'center', padding: 16,
    backgroundColor: '#202C33bf', borderBottomColor: '#1f2c34',
    borderBottomWidth: StyleSheet.hairlineWidth,
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.08, shadowRadius: 2, elevation: 1,
  },
  headerAvatar: {
    width: 38, height: 38, borderRadius: 19, backgroundColor: '#34B7F1', marginRight: 3,
  },
  ...
  inputContainer: {
    flex: 1, backgroundColor: '#202C33', borderRadius: 24, paddingHorizontal: 15, minHeight: 50,
    marginRight: 8, shadowColor: '#34B7F1', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.23, shadowRadius: 2.62, elevation: 2,
  },
  textInput: { flex: 1, color: '#fff', fontSize: 16, paddingVertical: 10, fontWeight: '400' },
  sendButtonContainer: {
    width: 50, height: 50, borderRadius: 25, overflow: 'hidden', justifyContent: 'center', alignItems: 'center', marginLeft: 0, marginBottom: 3,
  },
  gradientButton: { width: 50, height: 50, borderRadius: 25, justifyContent: 'center', alignItems: 'center' },
  datePillContainer: {
    backgroundColor: '#1f2c34ee', borderRadius: 9, paddingVertical: 6, paddingHorizontal: 14,
    alignSelf: 'center', marginVertical: 10, shadowColor: '#34B7F1', shadowOffset: { width: 0, height: 2 },
