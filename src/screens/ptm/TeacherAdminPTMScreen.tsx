...
const TeacherAdminPTMScreen = () => {
  ...
  // ✅ ADDED meeting_link to initial state
  const initialFormState = { meeting_datetime: '', teacher_id: '', subject_focus: '', status: 'Scheduled', notes: '', meeting_link: '' };
  ...
  const handleOpenModal = (meeting: Meeting | null = null) => {
    ...
    if (meeting) {
      setFormData({
          ...
          notes: meeting.notes || '',
          meeting_link: meeting.meeting_link || '', // ✅ Set meeting link on edit
      });
    } else {
  ...
  const handleSave = async () => {
    ...
    // ✅ ADDED meeting_link to the body
...
const styles = StyleSheet.create({ /* Styles remain the same */ center: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 20 }, container: { flex: 1, backgroundColor: '#f0f4f7' }, header: { flexDirection: 'row', alignItems: 'center', padding: 20, borderBottomWidth: 1, borderBottomColor: '#e2e8f0', backgroundColor: 'white' }, headerIcon: { fontSize: 32, marginRight: 15, color: '#5a67d8' }, headerTitle: { fontSize: 22, fontWeight: 'bold', color: '#2d3748' }, headerSubtitle: { fontSize: 14, color: '#718096' }, scheduleBtn: { backgroundColor: '#4CAF50', padding: 15, borderRadius: 8, marginHorizontal: 20, marginVertical: 10, alignItems: 'center' }, scheduleBtnText: { color: 'white', fontSize: 16, fontWeight: 'bold' }, emptyText: { textAlign: 'center', marginTop: 30, fontSize: 16, color: '#718096' }, modalBackdrop: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(0,0,0,0.5)' }, modalContent: { backgroundColor: 'white', borderRadius: 10, padding: 20, width: '90%', maxHeight: '80%' }, modalTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 20, textAlign: 'center' }, label: { fontSize: 16, fontWeight: '500', color: '#333', marginBottom: 5 }, input: { borderWidth: 1, borderColor: '#ccc', padding: 10, borderRadius: 5, marginBottom: 15, backgroundColor: '#fff', color: '#000' }, pickerContainer: { borderWidth: 1, borderColor: '#ccc', borderRadius: 5, marginBottom: 15, backgroundColor: '#fff' }, modalActions: { flexDirection: 'row', justifyContent: 'space-around', marginTop: 20, borderTopWidth: 1, borderTopColor: '#eee', paddingTop: 20 }, modalButton: { paddingVertical: 10, paddingHorizontal: 30, borderRadius: 5 }, cancelButton: { backgroundColor: '#e0e0e0' }, saveButton: { backgroundColor: '#4CAF50' }, saveButtonText: { color: 'white', fontWeight: 'bold' }});
