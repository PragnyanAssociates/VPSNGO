...
// --- Assignment List View Component (This section is unchanged) ---
const AssignmentList = ({ onSelectAssignment }) => {
    ...
    const fetchTeacherAssignments = useCallback(async () => { if (!user) return; setIsLoading(true); try { const response = await fetch(`${API_BASE_URL}/api/homework/teacher/${user.id}`); if (!response.ok) throw new Error("Failed to fetch assignment history."); setAssignments(await response.json()); } catch (e) { Alert.alert("Error", e.message); } finally { setIsLoading(false); } }, [user]);
    const fetchStudentClasses = async () => { try { const response = await fetch(`${API_BASE_URL}/api/student-classes`); if(response.ok) setStudentClasses(await response.json()); } catch (e) { console.error("Error fetching student classes:", e); } };
    useEffect(() => { fetchTeacherAssignments(); fetchStudentClasses(); }, [fetchTeacherAssignments]);
    const handleClassChange = async (classGroup) => { setSelectedClass(classGroup); setSubjects([]); setSelectedSubject(''); if (classGroup) { try { const response = await fetch(`${API_BASE_URL}/api/subjects-for-class/${classGroup}`); const data = await response.json(); setSubjects(data); return data; } catch (e) { console.error(e); return []; } } return []; };
    ...
    const openEditModal = async (assignment) => { setEditingAssignment(assignment); const date = new Date(assignment.due_date); const formattedDate = date.toISOString().split('T')[0]; setNewAssignment({ title: assignment.title, description: assignment.description, due_date: formattedDate }); setAttachment(assignment.attachment_path ? { name: assignment.attachment_path.split('/').pop() } : null); const fetchedSubjects = await handleClassChange(assignment.class_group); if (fetchedSubjects.includes(assignment.subject)) { setSelectedSubject(assignment.subject); } setIsModalVisible(true); };
    const handleDelete = (assignment) => { Alert.alert("Confirm Delete", `Are you sure you want to delete "${assignment.title}"?`, [ { text: "Cancel", style: 'cancel' }, { text: "Delete", style: 'destructive', onPress: async () => { try { const response = await fetch(`${API_BASE_URL}/api/homework/${assignment.id}`, { method: 'DELETE' }); if (!response.ok) throw new Error('Failed to delete assignment.'); Alert.alert("Success", "Assignment deleted."); fetchTeacherAssignments(); } catch(e) { Alert.alert("Error", e.message); } }}, ]); };
    const handleSave = async () => { const isEditing = !!editingAssignment; const url = isEditing ? `${API_BASE_URL}/api/homework/${editingAssignment.id}` : `${API_BASE_URL}/api/homework`; if (!user || !selectedClass || !selectedSubject || !newAssignment.title || !newAssignment.due_date) { return Alert.alert("Validation Error", "Title, Class, Subject, and Due Date are required."); } setIsSaving(true); const formData = new FormData(); formData.append('title', newAssignment.title); formData.append('description', newAssignment.description || ''); formData.append('due_date', newAssignment.due_date); formData.append('class_group', selectedClass); formData.append('subject', selectedSubject); if (!isEditing) { formData.append('teacher_id', user.id); } if (attachment && attachment.uri) { formData.append('attachment', { uri: attachment.uri, type: attachment.type, name: attachment.name }); } else if (isEditing && editingAssignment.attachment_path) { formData.append('existing_attachment_path', editingAssignment.attachment_path); } try { const response = await fetch(url, { method: 'POST', body: formData, headers: {'Content-Type': 'multipart/form-data'} }); const resData = await response.json(); if (!response.ok) throw new Error(resData.message || "An error occurred."); Alert.alert("Success", `Assignment ${isEditing ? 'updated' : 'created'}!`); setIsModalVisible(false); fetchTeacherAssignments(); } catch (e) { Alert.alert("Error", e.message); } finally { setIsSaving(false); } };
...
const SubmissionList = ({ assignment, onBack }) => {
    ...
    const fetchStudentRoster = useCallback(async () => {
        ...
        try {
            ...
            setStudentRoster(await response.json());
    ...
    const handleGrade = async () => {
        ...
        setIsGrading(true);
        try {
            const response = await fetch(`${API_BASE_URL}/api/homework/grade/${selectedSubmission.submission_id}`, {
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                data={studentRoster}
                // ✅ FIX: Made the keyExtractor safe to prevent crashes from bad data
