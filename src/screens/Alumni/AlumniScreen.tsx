...
// ★★★ 1. CORRECT IMPORTS: Use apiClient for API calls ★★★
...
const AlumniScreen: React.FC = () => {
    const { token } = useAuth(); // Keep token for now if needed, though apiClient handles it
    ...
    const fetchData = useCallback(async () => {
        try {
            setLoading(true);
            // ★★★ 2. CORRECTED API CALL: Use apiClient ★★★
    ...
    const handleSave = async () => {
        ...
        try {
            // ★★★ 2. CORRECTED API CALL: Use apiClient for both create and update ★★★
            let response;
    ...
    return (
        <View style={styles.container}>
            <FlatList
                ...
                ListEmptyComponent={<View style={styles.emptyContainer}><MaterialIcons name="school" size={80} color="#CFD8DC" /><Text style={styles.emptyText}>No Alumni Found</Text><Text style={styles.emptySubText}>Tap the '+' button to add a record.</Text></View>}
            ...
            <Modal visible={modalVisible} animationType="slide" onRequestClose={() => setModalVisible(false)}>
                <ScrollView style={styles.modalContainer} contentContainerStyle={{paddingBottom: 50}}>
                    ...
                    <View style={styles.imagePickerContainer}>
                        {/* ★★★ 3. CORRECTED IMAGE PATH: Use native drawable resource ★★★ */}
                        <Image source={selectedImage?.uri ? { uri: selectedImage.uri } : (formData.profile_pic_url ? { uri: `${SERVER_URL}${formData.profile_pic_url}` } : { uri: 'default_avatar' })} style={styles.profileImage} />
...
const AlumniCardItem = ({ item, onEdit, onDelete, isExpanded, onPress }) => (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.8}>
        <View style={styles.cardHeader}>
            {/* ★★★ 3. CORRECTED IMAGE PATH: Use native drawable resource ★★★ */}
            <Image source={item.profile_pic_url ? { uri: `${SERVER_URL}${item.profile_pic_url}` } : { uri: 'default_avatar' }} style={styles.avatarImage} />
            <View style={styles.cardHeaderText}><Text style={styles.cardTitle}>{item.alumni_name}</Text><Text style={styles.cardSubtitle}>Admission No: {item.admission_no}</Text></View>
            <View style={styles.cardActions}>{item.present_status && <View style={styles.statusTag}><Text style={styles.statusTagText}>{item.present_status}</Text></View>}<View style={styles.buttonGroup}><TouchableOpacity onPress={() => onEdit(item)} style={styles.iconButton}><FontAwesome name="pencil" size={18} color="#FFA000" /></TouchableOpacity><TouchableOpacity onPress={() => onDelete(item.id)} style={styles.iconButton}><FontAwesome name="trash" size={18} color="#D32F2F" /></TouchableOpacity></View></View>
        </View>
        <View style={styles.cardBody}><InfoRow icon="phone" label="Phone" value={item.phone_no || 'N/A'} /><InfoRow icon="calendar-plus-o" label="Joined" value={`${formatDate(item.school_joined_date)} (${item.school_joined_grade || 'N/A'})`} /><InfoRow icon="calendar-times-o" label="Left" value={`${formatDate(item.school_outgoing_date)} (${item.school_outgoing_grade || 'N/A'})`} /></View>
        {isExpanded && (<View style={styles.expandedContainer}><InfoRow icon="birthday-cake" label="D.O.B" value={formatDate(item.dob)} /><InfoRow icon="id-card-o" label="Pen No" value={item.pen_no || 'N/A'} /><InfoRow icon="vcard" label="Aadhar" value={item.aadhar_no || 'N/A'} /><InfoRow icon="user" label="Parent" value={item.parent_name || 'N/A'} /><InfoRow icon="mobile" label="Parent No" value={item.parent_phone || 'N/A'} /><InfoRow icon="file-text-o" label="TC No" value={item.tc_number || 'N/A'} /><InfoRow icon="calendar-check-o" label="TC Issued" value={formatDate(item.tc_issued_date)} /><InfoRow icon="map-marker" label="Address" value={item.address || 'N/A'} isMultiLine={true} /></View>)}
    </TouchableOpacity>
);
...
const styles = StyleSheet.create({ center: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#F0F4F7' }, container: { flex: 1, backgroundColor: '#F0F4F7' }, header: { flexDirection: 'row', alignItems: 'center', padding: 16, backgroundColor: '#FFFFFF', borderRadius: 12, marginVertical: 10, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, }, headerIconContainer: { width: 45, height: 45, borderRadius: 22.5, backgroundColor: '#E0F2F1', justifyContent: 'center', alignItems: 'center', marginRight: 16 }, headerTitle: { fontSize: 20, fontWeight: 'bold', color: '#004D40' }, headerSubtitle: { fontSize: 14, color: '#00796B' }, card: { backgroundColor: '#FFFFFF', borderRadius: 12, marginVertical: 8, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.1, shadowRadius: 4, overflow: 'hidden' }, cardHeader: { flexDirection: 'row', alignItems: 'flex-start', padding: 16 }, avatarImage: { width: 50, height: 50, borderRadius: 25, marginRight: 12, backgroundColor: '#E0E0E0' }, cardHeaderText: { flex: 1, justifyContent: 'center', marginTop: 4 }, cardTitle: { fontSize: 17, fontWeight: 'bold', color: '#212121' }, cardSubtitle: { fontSize: 13, color: '#757575', marginTop: 2 }, cardActions: { alignItems: 'flex-end' }, buttonGroup: { flexDirection: 'row', marginTop: 8 }, iconButton: { marginLeft: 16 }, statusTag: { backgroundColor: '#4CAF50', paddingHorizontal: 12, paddingVertical: 5, borderRadius: 15 }, statusTagText: { color: '#fff', fontSize: 11, fontWeight: 'bold' }, cardBody: { paddingHorizontal: 16, paddingBottom: 16, paddingTop: 0 }, expandedContainer: { paddingHorizontal: 16, paddingBottom: 16, borderTopWidth: 1, borderTopColor: '#EEEEEE' }, infoRow: { flexDirection: 'row', alignItems: 'center', marginTop: 12 }, infoIcon: { width: 24, textAlign: 'center' }, infoLabel: { fontSize: 14, fontWeight: '600', color: '#424242', marginLeft: 10 }, infoValue: { fontSize: 14, color: '#757575', flex: 1, marginLeft: 5, flexWrap: 'wrap' }, fab: { position: 'absolute', right: 25, bottom: 25, width: 56, height: 56, borderRadius: 28, backgroundColor: '#0288D1', justifyContent: 'center', alignItems: 'center', elevation: 8, shadowColor: '#000', shadowRadius: 5, shadowOpacity: 0.3 }, emptyContainer: { alignItems: 'center', justifyContent: 'center', marginTop: '30%', opacity: 0.6 }, emptyText: { fontSize: 18, fontWeight: '600', color: '#78909C', marginTop: 16 }, emptySubText: { fontSize: 14, color: '#78909C', marginTop: 4 }, modalContainer: { flex: 1, backgroundColor: '#f9f9f9', paddingTop: Platform.OS === 'ios' ? 50 : 20, paddingHorizontal: 20 }, modalTitle: { fontSize: 24, fontWeight: 'bold', marginBottom: 25, textAlign: 'center', color: '#212121' }, label: { fontSize: 16, color: '#555', marginBottom: 8, marginTop: 12, fontWeight: '600' }, input: { backgroundColor: '#FFFFFF', borderWidth: 1, borderColor: '#CFD8DC', paddingVertical: 12, paddingHorizontal: 15, borderRadius: 8, marginBottom: 10, fontSize: 16, color: '#333' }, dateText: { color: '#333', fontSize: 16, paddingVertical: 4 }, textArea: { height: 100, textAlignVertical: 'top' }, modalActions: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 30, marginBottom: 50 }, modalButton: { paddingVertical: 14, borderRadius: 8, flex: 1, alignItems: 'center', elevation: 2 }, cancelButton: { backgroundColor: '#9E9E9E', marginRight: 10 }, saveButton: { backgroundColor: '#0288D1', marginLeft: 10 }, modalButtonText: { color: 'white', fontWeight: 'bold', fontSize: 16 }, imagePickerContainer: { alignItems: 'center', marginBottom: 20 }, profileImage: { width: 120, height: 120, borderRadius: 60, backgroundColor: '#E0E0E0', marginBottom: 10, borderWidth: 3, borderColor: '#0288D1' }, imagePickerButton: { flexDirection: 'row', backgroundColor: '#0288D1', paddingVertical: 10, paddingHorizontal: 20, borderRadius: 8, alignItems: 'center' }, imagePickerButtonText: { color: '#fff', marginLeft: 10, fontWeight: 'bold' }, });
