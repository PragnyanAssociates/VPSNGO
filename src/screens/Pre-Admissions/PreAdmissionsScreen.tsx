...
const PreAdmissionsScreen: React.FC = () => {
    ...
    const handleSave = async () => {
        if (!formData.admission_no || !formData.student_name || !formData.joining_grade) {
            return Alert.alert('Validation Error', 'Admission No, Student Name, and Joining Grade are required.');
        }
        ...
        Object.keys(processedData).forEach(key => {
            const value = processedData[key as keyof PreAdmissionRecord];
            if (value !== null && value !== undefined) body.append(key, String(value));
        });

        if (selectedImage?.uri) {
            body.append('photo', { uri: selectedImage.uri, type: selectedImage.type, name: selectedImage.fileName });
        }
        ...
        Alert.alert("Confirm Delete", "Are you sure you want to delete this record?", [
            ...
            { text: "Delete", style: "destructive", onPress: async () => {
                try {
                    ...
                    Alert.alert("Success", resData.message);
                    fetchData();
                } catch (error: any) { Alert.alert('Delete Error', error.message); }
            }}
        ]);
    };
            ...
            <FlatList
                ListHeaderComponent={
                    <View style={styles.header}>
                        <View style={styles.headerIconContainer}><MaterialIcons name="person-add-alt-1" size={24} color="#00796B" /></View>
                        <View><Text style={styles.headerTitle}>Pre-Admissions</Text><Text style={styles.headerSubtitle}>Manage admission applications</Text></View>
                    </View>
                }
                data={data}
                ...
                ListEmptyComponent={
                  <View style={styles.emptyContainer}>
                    <MaterialIcons name="inbox" size={80} color="#CFD8DC" />
                    <Text style={styles.emptyText}>No applications found.</Text>
                    <Text style={styles.emptySubText}>Tap '+' to add a new application.</Text>
                  </View>
                }
...
const styles = StyleSheet.create({
    ...
    header: { flexDirection: 'row', alignItems: 'center', padding: 16, backgroundColor: '#FFFFFF', borderRadius: 12, marginVertical: 10, marginHorizontal: 16, elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, },
    ...
    avatarImage: { width: 45, height: 45, borderRadius: 22.5, marginRight: 12, backgroundColor: '#E0E0E0' },
    ...
    statusPill: { borderRadius: 12, paddingVertical: 5, paddingHorizontal: 12, alignItems: 'center' },
    statusPillText: { fontSize: 11, fontWeight: 'bold' },
    expandedContainer: { paddingHorizontal: 16, paddingBottom: 16, paddingTop: 8, borderTopWidth: 1, borderTopColor: '#EEEEEE' },
